...
contract Receiver is CCIPReceiver, OwnerIsCreator {
    ...
    function _ccipReceive(
        ...
        (address _beneficiary, uint _amount) = abi.decode(
            any2EvmMessage.data,
            ((address), (uint))
        );

        // (bool success) = i_usdcToken.transfer(_beneficiary,_amount);
        i_usdcToken.safeTransferFrom(
            msg.sender,
            _beneficiary,
            _amount
        );
        // (bool success, bytes memory returnData) = address(i_usdcToken).call(
        //     any2EvmMessage.data
