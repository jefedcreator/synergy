...
contract Pool is IPool, ERC20Permit, ReentrancyGuard {
    ...
    function mint(
        ...
        (, uint256 _amount0) = _balance0.trySub(_reserve0);
        (, uint256 _amount1) = _balance1.trySub(_reserve1);
        uint256 _totalSupply = totalSupply(); // gas savings, must be defined here since totalSupply can update in _mintFee
        if (_totalSupply == 0) {
            // Use ABDKMath64x64 for precise square root calculation
            (, uint256 _amount) = _amount0.tryMul(_amount1);
            bytes16 amountQuad = ABDKMathQuad.fromUInt(_amount);
            bytes16 sqrtKQuad = ABDKMathQuad.sqrt(amountQuad);
            uint256 sqrtKUint = ABDKMathQuad.toUInt(sqrtKQuad);
            (, liquidity) = sqrtKUint.trySub(MINIMUM_LIQUIDITY);

            _mint(address(1), MINIMUM_LIQUIDITY); // permanently lock the first MINIMUM_LIQUIDITY tokens - cannot be address(0)
            if (stable) {
                (, uint256 amount0) = _amount0.tryMul(1e18);
                (, uint256 amount1) = _amount1.tryMul(1e18);
                (, uint _decimals0) = amount0.tryDiv(decimals0);
                (, uint _decimals1) = amount1.tryDiv(decimals1);
                if (_decimals0 != _decimals1) revert DepositsNotEqual();
                if (_k(_amount0, _amount1) <= MINIMUM_K) revert BelowMinimumK();
            }
        } else {
            (, uint256 amount0MulSupply) = _amount0.tryMul(_totalSupply);
            (, uint256 amount1MulSupply) = _amount1.tryMul(_totalSupply);
            (, uint256 liquidity0) = amount0MulSupply.tryDiv(_reserve0);
            (, uint256 liquidity1) = amount1MulSupply.tryDiv(_reserve1);
